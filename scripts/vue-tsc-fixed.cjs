...
fs.readFileSync = (...args) => {
  if (args[0] === tscPath) {
    ...
    function tryReplace(search, replace, options = { required: true }) {
      const before = tsc
      tsc = tsc.replace(search, replace)
      const after = tsc
      if (options.required && after === before) {
...
;(function main() {
  try {
    // Avoid stale incremental diagnostics with patched vue-tsc + TS 5.7.
